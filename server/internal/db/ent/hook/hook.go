...
// The DashboardAgentFunc type is an adapter to allow the use of ordinary
// function as DashboardAgent mutator.
type DashboardAgentFunc func(context.Context, *ent.DashboardAgentMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f DashboardAgentFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.DashboardAgentMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.DashboardAgentMutation", m)
}
