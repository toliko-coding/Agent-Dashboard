...
- **Network** — a count of active connections on the Overview and the System Map, driven by the collector's own number. There is no detail view yet; if the collector reports `null` for it the card stays unavailable rather than reading `0`.
