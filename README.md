...
LocalScope is a separate project and an optional dependency. The dashboard works without it; when it is running, the Command page's status strip and Runtime summary, the runtime topology, and the **LocalScope** view fill in with real data.
...
- **Devices** — Android emulators and devices, and iOS simulators. "No adapter could run" (adb missing) and "zero devices connected" are reported as different facts: a count is only shown when an adapter actually ran, and an adapter reporting *partial* results — uninstalled simulator runtimes, say — still counts as having run, so it never erases the devices that are genuinely there.
- **Network** — a count of active connections in the Command page's Runtime summary, driven by the collector's own number. There is no detail view yet; if the collector reports `null` for it the card stays unavailable rather than reading `0`.
