...
- **Services** — one row per listening port: the classification in developer terms ("Vite Development Server"), its runtime, and its workspace. A low-confidence classification is marked a guess, and a service bound to all interfaces is flagged, because that is reachable from your network. Each TCP port reachable on this machine is a link that opens `http://localhost:<port>` in a new tab, built from the port alone; other ports stay plain text. Address, PID, protocol and classification are behind each row's Details.
