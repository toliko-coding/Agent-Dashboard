...
Agents are named the way Claude Code names their sessions: the name set with `/rename`, otherwise the title Claude Code generated for the session. A pipeline task's title comes first, and the provider and short session id are the fallback. A small outlined glyph marks what kind of worker each agent is: pipeline task agent, Claude Code internal process, desktop app session, a terminal session the dashboard can attach to, or a command-line session. The **Agents** view switches between a compact, responsive **Grid** and a **List** with a toggle in its toolbar. Motion only ever means something is happening: a card's top edge breathes while the agent works and shows a short sweep while a tool call is open, a **Needs you** chip pulses once as it arrives, and a failure shows a still red edge. Nothing moves while agent updates are reconnecting or when reduced motion is requested.
...
- **Claude usage**: tokens and estimated cost over the last 5 hours and the last 7 days, taken from the Claude session logs on this machine. It shows a budget share only when you have set a budget, and it does not show your plan's rate limits.
