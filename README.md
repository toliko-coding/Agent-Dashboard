...
Below it, **Active work** lists only the agents executing right now, framed by repository and nested by workspace (opaque identity, never folder names), each as one row: state, name, the tool it is using, model, role, subagents and last activity — no paths, PIDs, commands or transcript. A status strip above both reads Needs you, Working, where agents are, and the local runtime; the **Runtime** section summarises services, processes, devices and network from the shared LocalScope snapshot, with the runtime topology collapsed beneath it. Recent activity (the server audit log), Pipeline, Projects, Routines, Memory and GitHub follow as secondary panels.
