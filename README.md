...
An agent can be given a **name** and an **icon** when you start it from **New Agent** (for example "Resume Editor" with the Documents icon). Both are optional, work without a Project, and are presentation only. They are saved by session id and survive reloads and restarts. Without a name, an agent shows its Claude Code session title (the name set with `/rename`, otherwise the generated title), and without that, its provider and short session id; a pipeline task's title comes before the session title. A named agent shows its session title as what it is working on and "Claude · <short session id>" as a technical handle. Without a chosen icon, every agent shows the same neutral General icon; how it was launched is shown in the handle's tooltip. The **Agents** view shows rich cards (three to a row on a desktop), or a dense **List**. Cards show uptime, recent tool calls, tokens, estimated cost, a breakdown of recent tool calls by tool and the actions; opening an agent shows a large centered workspace with the conversation as its main area. **Stop** and **Delete** always ask for confirmation. Deleting removes the agent from the dashboard but never its project files, Git repository or Claude history. In **New Agent**, choose **New projectless workspace** to have the dashboard create a plain folder for the agent inside the projectless agents folder, which you can change in **Settings → Agent folders**. Motion only ever means something is happening: a card's top edge breathes while the agent works and shows a short sweep while a tool call is open, a **Needs you** chip pulses once as it arrives, and a failure shows a still red edge. Nothing moves while agent updates are reconnecting or when reduced motion is requested.

The **Command** page (the landing view) opens with **Needs you**: one queue of what is waiting on your decision, ranked blocking (a question or answer-submit screen on an agent's terminal, a held or stored permission request, the session's own permission prompt, a capability decision), then failed (a classified API error — rate limited, quota exhausted, authentication failed — on a session that is not working), then ready (a pipeline task whose stage run is waiting for you). Each row names the agent, the reason, the repository and workspace when they are resolved, and how long the request has waited when the request carries a time; selecting it opens the agent's details or the task. It never shows paths, commands or transcript text. An idle agent, a working one, a dropped agent stream or an unavailable LocalScope is not attention, and there is no "stalled" guess — the data cannot tell a stuck session from a long-running tool. Empty, it is a single line. The sidebar's Agents badge and the Agents view's triage band read the same queue, so all three show the same items and the same count; the topbar and sidebar say only whether agent updates are live.

Below it, **Active work** lists only the agents executing right now, framed by repository and nested by workspace (opaque identity, never folder names), each as one row: state, category glyph and name, the tool it is using, model, role, subagents, how long its process has been running, and last activity — no paths, PIDs, commands or transcript. A status bar above both shows Needs you, Working (and how many agents are using tools), running sessions, where agents are, and the local runtime. The **Runtime** section summarises services, processes, devices and network from the shared LocalScope snapshot, with the runtime topology collapsed beneath it: a tree of repositories, workspaces, agents, processes and services. Beside the work, a rail shows:

- **Claude usage**: tokens and estimated API-equivalent cost over the last 5 hours and the last 7 days, taken from the Claude session logs on this machine. It shows a budget share only when you have set a budget. It is not an Anthropic bill and does not show your plan's usage limits. Running sessions' whole-life cost is shown separately, labelled **Active sessions · lifetime**, because it is not comparable with the time windows.
- **This machine**: CPU, memory and disk as the dashboard server measures them.
- **Recent activity**: the server audit log as a timeline.

Pipeline, Projects, Routines, Memory and GitHub follow as secondary panels.
...
**Build & control** — multi-stage task pipeline (`concept → implementation → review → done`) running in isolated git worktrees behind per-stage permission gates; per-stage engine and model selection (global and per-project), plus a per-stage reasoning-effort setting for the `claude` adapter (`low` through `max`, reaching the spawned CLI process as `--effort`) — always shown, disabled with a reason for adapters that don't support it; spawn agents and send follow-ups or `/btw` interrupts via MCP channels; refinement chat to shape a concept first; an opt-in plan-review stage that auto-generates and self-reviews the implementation plan and gates on your approval before any files are edited; seed a task straight from a GitHub or Jira issue — paste a reference into the New-Task form's **Import from issue** field to pre-fill the title and description (tracker tokens are stored encrypted and managed under **Settings → Tracker**); shared scratchpads and lease-based locks for agent coordination (`agent:coord` MCP scope); per-turn worktree checkpoints with one-click revert (a debounced filesystem watcher snapshots each agent turn into a hidden git ref; revert restores the worktree and parks the task for manual resume); drag-and-drop task reordering. A task's **Autonomy** says how much its stage agents may do without asking: new tasks start at **Manual** (only what the task was granted; every other permission request waits for you in **Needs you**), while **Spec-gated** and **Full** pre-approve every tool — shell commands and file writes in the task's workspace included — and approve every permission request automatically, with only `git push` held back. A task's working folder must be a project or allowed folder and never a sensitive directory, whatever its autonomy, and a failed stage run records why (could not start, needed a permission, failed, disappeared, workspace unavailable, timed out, invalid result, cancelled). See [Task autonomy](docs/guides/agent-control.md#task-autonomy).
...
A new agent needs only a working folder — a repository checkout, a worktree, or a plain folder; a Dashboard Project and a GitHub remote are optional. The dashboard starts agents only inside folders you have allowed (project folders, or folders allowed from the New Agent dialog), and when Claude Code asks whether to trust a new folder, that question is shown to you — in the dialog, then in **Needs you** — and answered only by you. See [Controlling & spawning agents](docs/guides/agent-control.md#spawning-new-agents).

Each Project has a **roadmap** (Projects → Roadmap): its objective, where it is now, what comes next, and phases and items marked User, Suggested or Verified by the server. **Analyze with AI** starts a Project Intelligence agent that reads the project and proposes a roadmap; you review it as **Current vs Suggested** — each phase New, Changed, Not in proposal or Unchanged — then **Add** only the new phases, **Replace** the roadmap after confirming, or **Reject** it. Nothing changes until you choose.

An agent Agent Dashboard started has an **Open terminal** action in its workspace, on its card and in its Connections panel — a real `xterm.js` terminal streamed over a WebSocket (`GET /api/agents/{pid}/terminal`), so you see the session's actual pty output and can type into it directly. When Claude stops at its own permission prompt, **Needs you** and the workspace show the tool and command or file path with **Deny** and **Approve once** — one request, never "don't ask again" or auto mode, re-checked on the server against the prompt actually open — and **Respond in terminal** for prompts the dashboard cannot map with confidence. The dashboard never answers on its own. Keystrokes are control, so a terminal is attached only to sessions the dashboard launched (owned agents and pipeline stage agents); a Terminal or VS Code session, or one started with `agent-dashboard live`, stays observe-only and the server refuses the attach. When the session asks an interactive multiple-choice question (`AskUserQuestion`), an overlay detects it from the live terminal screen and renders the options inline — pick them and **Send answer** drives the session's selector with real keystrokes over the same WebSocket. A multi-question flow's closing **review/submit** screen ("Ready to submit your answers?") is detected the same way, so the whole round can be completed without touching the terminal. Because it drives the pty directly, this works over any transport (tmux or the built-in pty broker). The same cards appear in the main needs-you band, backed by server-side detection of the session's rendered screen. The workspace header also shows the permission mode the running session was started with, read from the process itself and marked when it is a dangerous one — Claude reads its mode once, at startup, so it describes the session as it is running rather than a setting that can be changed under it. An agent is a durable thing rather than a session: its name, its standing instructions and its saved permission mode live on the agent and outlive the Claude process, so a finished agent still has them and can be resumed with them. **Edit agent** covers Identity, Instructions, Permissions and Session behaviour, and says which half you are changing — Claude reads its mode and system prompt once, at startup, so saving describes the next session and never the one already running. Resuming shows the agent, workspace, permission mode and instructions first and starts nothing until you confirm; what the dialog displayed is checked against what is saved, so a configuration changed in another tab is refused rather than launched. An agent that has no session running is not gone: agents this dashboard keeps are read from storage and listed under **Agents with no session running** at the bottom of the Agents page, with their folder, saved permission mode and whether they have instructions, so a finished agent survives a server restart instead of vanishing with the in-memory finished cards. Each one can be opened — showing its identity, folder, Project, saved instructions and permission mode, all editable — and run again. It says whether it would **Resume** or **Start**: if Claude still holds the transcript of the agent's last session the next one continues that conversation, and if it does not, a new conversation begins and the dialog says so. Starting or resuming attaches the new session to the same agent rather than creating a second one. **Delete agent** is the only thing that removes one, and it removes the dashboard's record and nothing else — the folder, the repository and the Claude session transcript stay where they are. Stopping a session is not deleting an agent. The agent that maintains Agent Dashboard itself is a seeded record shown above the roster as the **Main agent**; nothing designates one, no ordinary agent can become it, it cannot be deleted, and the role grants no permission, no authority over other agents and no exemption from the ownership rules above. Its session is usually the Claude running in your own editor, which the dashboard observes rather than owns: the panel says so and offers **Observe session**, the prompt box refuses to send there rather than quietly starting a second one, and starting it is an explicit **Start main agent** that shows the folder, the permission mode and whether instructions exist before anything runs.
...
**Local machine observability (optional)** — the dashboard can consume [LocalScope](https://github.com/toliko-coding/LocalScope), a separate read-only collector that reports this machine's listening services, developer processes, and connected devices. LocalScope owns system collection; the dashboard only visualises it — no `ps`/`lsof`/`adb` parsing happens here. Because the collector binds `127.0.0.1`, answers `Access-Control-Allow-Origin: null`, and validates the `Host` header as DNS-rebinding protection, a browser cannot call it directly: the dashboard proxies it same-origin under `/localscope/*` (Vite in development, `server/internal/api/localscope` in the embedded build). That proxy forwards **only** an allow-list of collector paths, only `GET`, with no client headers and no cookies passed upstream, so it is not a general reverse proxy into loopback. Set `DASHBOARD_LOCALSCOPE_PORT` to point at a non-default port, or `0` to disable the proxy entirely. The dashboard translates the collector's wire format into a model of its own (`/api/localscope/snapshot`, `/api/localscope/services`, `/api/localscope/processes`, `/api/localscope/devices`), so the UI never sees LocalScope's envelope, and every surface reads the same reading rather than sampling the collector separately. Four states, because they are four different claims: collected, collected with a source degraded, **stale** (true when it was collected, not known to be true now), and never collected at all. A count is never `0` and a list is never `[]` unless the collector said so — before anything has been collected the Command page's runtime counts read "not connected" and the Runtime page says the collector is not connected and how to start it; once the collector goes away the last reading stays on screen marked `stale · 3m ago`, on the Runtime page as much as on Command, because dropping it would turn "I cannot see this machine" into "this machine is empty". Each section carries its own state, so one category being unavailable never blanks the others. See [LocalScope](#localscope) below.
...
LocalScope is a separate project and an optional dependency. The dashboard works without it; when it is running, the Command page's status strip and Runtime summary, the runtime topology, and the **Runtime** page fill in with real data.
...
The **Runtime** page is this machine in one place. A line beside its section tabs says only whether LocalScope's reading is live, partial, stale (with its age) or not connected — never a claim about the machine's overall health. Its sections:

- **Overview** — counts of listening services, developer processes, devices and active network connections from the LocalScope snapshot (a measured `0` reads 0; a count LocalScope did not measure reads "Not collected"; an unavailable collector makes every count unknown and shows how to start it), where running agents are by repository and workspace, and this machine's CPU, memory, disk, load, uptime and build, which the dashboard server measures itself. Network throughput is not collected, and the page says so.
- **Workspaces** — the runtime topology: repository → workspace → agents, processes and services, grouped only by resolved identity, so two repositories with the same name and two worktrees of one repository stay separate. Anything without a resolved workspace is listed under "Workspace unknown".
- **Services** — one row per listening port: the classification in developer terms ("Vite Development Server"), its runtime, its workspace, and what Agent Dashboard actually knows about it. The dashboard's own server and the server delivering the page you are reading are marked **System · protected**, matched by process id against `GET /api/system/self` rather than by port number (13120 and 5173 are defaults, not identities); a service running in the same workspace as an agent the dashboard started is marked **Agent workspace**, which places it without claiming the dashboard started it; everything else is simply observed. Runtime shows services and does not stop them: a service carries no ownership record the server could re-verify at the moment of a click, so no row offers Stop or Restart. A low-confidence classification is marked a guess, and a service bound to all interfaces is flagged, because that is reachable from your network. Each TCP port reachable on this machine is a link that opens `http://localhost:<port>` in a new tab, built from the port alone; other ports stay plain text. Address, PID, protocol and classification are behind each row's Details.
- **Processes** — the developer-relevant subset by default (e.g. 23 of 915): name, runtime, ports, workspace, and CPU and memory only when reported. PID, running time, why a process was listed and its command line are behind Details; no working directory is shown. **Scan all processes** is an explicit, one-off diagnostic — it is never refreshed — and the render cap is stated rather than silently truncating the list.
- **Devices** — Android emulators and devices, and iOS simulators. "No adapter could run" (adb missing) and "zero devices connected" are reported as different facts: a count is only shown when every adapter ran, and an adapter reporting *partial* results — uninstalled simulator runtimes, say — still counts as having run, so it never erases the devices that are genuinely there.

Each list is fetched only while its section is open, and every list keeps its last rows, marked stale with their age, when LocalScope stops answering.
