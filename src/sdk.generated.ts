 ...
 */
export const AgentTitleCustom = 'custom' // set by the user with /rename
export const AgentTitleAI = 'ai' // generated by Claude Code for the session
/**
 * AgentTitle* name where Agent.Title came from.
 */
export type AgentTitle = typeof AgentTitleCustom | typeof AgentTitleAI
export const AgentCategoryDevelopment = 'development'
export const AgentCategoryWeb = 'web'
export const AgentCategoryDocument = 'document'
export const AgentCategoryResearch = 'research'
export const AgentCategoryRuntime = 'runtime'
export const AgentCategoryData = 'data'
export const AgentCategoryGeneral = 'general'
/**
 * AgentCategory* are the icon categories a user can give an agent when starting
 * it (Agent.Category). Presentation only: a category never grants, scopes or
 * authorizes anything. AgentCategoryGeneral is also the neutral fallback a
 * client draws when no category was given.
 */
export type AgentCategory = typeof AgentCategoryDevelopment | typeof AgentCategoryWeb | typeof AgentCategoryDocument | typeof AgentCategoryResearch | typeof AgentCategoryRuntime | typeof AgentCategoryData | typeof AgentCategoryGeneral
...
export interface Agent {
  ...
  lastActivity: string
  /**
   * Title is the session's name as Claude Code shows it: the name set with
   * /rename when there is one, otherwise the title Claude Code generated for
   * the session. "" when the log carries neither. TitleSource says which.
   */
  title?: string
  titleSource?: string
  /**
   * DisplayName is the name the user gave this agent when starting it from the
   ...
   */
  terminalPermission?: TerminalPermissionPrompt
  /**
 ...
 * would be wrong the moment either is configured differently or another process
 * squats one, and a wrong answer here is the kind that ends with someone
 * stopping the server they are reading the page from. So the server states its
 * identity as fact — the pid it runs as, the address it bound, the directory it
 * runs in — and everything a client concludes from it is an equality check
 * against an observed service, never an inference from a label.
 */
